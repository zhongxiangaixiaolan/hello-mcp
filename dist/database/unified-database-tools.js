...
export class UnifiedDatabaseTools {
     ...
     * 执行数据库操作
     */
    async executeDatabaseOperation(params) {
    ...
    async handleConnect(params) {
        ...
        if (!connectionConfig) {
            ...
            if (defaultConnections[params.connectionId]) {
                ...
                throw new DatabaseError(`Connection configuration is required when connectionConfig is not provided. ${availableConnectionsText}. To use pre-configured connections, ensure they are defined in settings.json under databaseOperation.defaultConnections.`, 'MISSING_CONNECTION_CONFIG');
            }
        }
     ...
     * 处理列出数据库操作
     */
    async handleListDatabases(params) {
     ...
     * 处理描述表操作
     */
    async handleDescribeTable(params) {
        if (!params.tableName) {
...
//# sourceMappingURL=unified-database-tools.js.map